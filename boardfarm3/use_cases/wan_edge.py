...
- :func:`assert_active_path` — assert the device is forwarding on the expected WAN label.
- :func:`assert_path_steers_on_impairment` — inject a blackout and assert the device steers
...
- :func:`assert_path_metrics_within_slo` — assert device-reported path metrics are within SLO.
...
def get_wan_edge(name: str | None = None) -> WANEdgeDevice:
    ...
    Supports single-device (omit *name*) and multi-device (specify *name*) topologies.
...
def assert_active_path(
    ...
    *,
    ...
    """Assert the device is currently forwarding traffic on the expected WAN interface.

    .. hint:: Implements steps such as:
        - Then traffic should be forwarded on wan1
        - Then the active path should be wan2

    The most direct path assertion — reads the device's current forwarding state
    ...
    :param edge: WANEdgeDevice instance.
    ...
    :param flow_dst: Optional destination IP to select a specific flow path
        (used when the device has per-flow or per-application steering).
    ...
    """
    active = edge.get_active_wan_interface(flow_dst=flow_dst)
    prefix = f"[{label}] " if label else ""
    assert active == expected_wan, (
        f"{prefix}Expected active path {expected_wan!r} but device reports {active!r}"
    )
...
def assert_path_steers_on_impairment(
    ...
    *,
    ...
    """Apply blackout impairment to a WAN link and assert the device steers to the expected fallback.
    ...
    (self-restoring after *duration_ms*).  Polls until the device's active path changes to
    *expected_fallback_wan*, then verifies the correct fallback was selected — not just
    ...
    device chose the right path in response to the impairment.
    ...
    :param expected_fallback_wan: Logical label the device should steer to (e.g. ``"wan2"``).
    :param duration_ms: How long the blackout lasts before auto-restore (ms).
    :param poll_interval_ms: Polling interval while waiting for device to switch (ms).
    :param timeout_ms: Maximum wait for path switch before failing (ms).
    :param label: Optional scenario label for clearer assertion messages.
    :raises AssertionError: if device does not switch to *expected_fallback_wan* within
    ...
    while time.monotonic() < deadline:
        active = edge.get_active_wan_interface()
        if active != impaired_wan:
            assert active == expected_fallback_wan, (
                f"{prefix}Device steered away from {impaired_wan!r} but chose "
                f"{active!r} instead of expected fallback {expected_fallback_wan!r}"
            )
            return
        time.sleep(poll_interval_ms / 1000)
    raise AssertionError(
        f"{prefix}Device did not steer away from impaired {impaired_wan!r} "
...
def assert_policy_steered_path(
    ...
    *,
    ...
    """Apply a PBR policy and assert the device routes the specified flow via the expected WAN.
    ...
    """
    prefix = f"[{label}] " if label else ""
    edge.apply_policy(policy)
    active = edge.get_active_wan_interface(flow_dst=flow_dst)
    assert active == expected_wan, (
        f"{prefix}Policy {policy} should steer {flow_dst!r} to {expected_wan!r} "
        f"but device reports active path {active!r}"
    )
...
def assert_wan_interface_status(
    ...
    *,
    ...
    Reads the device's interface status table and asserts the named WAN link
    ...
    """
    prefix = f"[{label}] " if label else ""
    statuses = edge.get_wan_interface_status()
    assert wan_label in statuses, (
        f"{prefix}WAN label {wan_label!r} not found in device interface status. "
...
def assert_path_metrics_within_slo(
    ...
    *,
    ...
    """Assert the device's measured path metrics for a WAN link are within defined thresholds.
    ...
    on the Linux Router this is implemented via active ping probes; on commercial appliances
    ...
    """
    prefix = f"[{label}] " if label else ""
    all_metrics = edge.get_wan_path_metrics()
...
def wait_for_path_switch(
    ...
    *,
    ...
    """Poll until the device's active forwarding path matches *expected_wan*.
    ...
    active = edge.get_active_wan_interface()
    raise AssertionError(
        f"{prefix}Device did not switch to {expected_wan!r} within {timeout_ms}ms "
...
def measure_failover_convergence(
    ...
    """Inject a blackout on *primary_link* and measure time until device switches to *backup_link*.

    .. hint:: Implements steps such as:
        - Then the device should switch to the backup path within 1000ms
    ...
    until the device reports *backup_link* as active.
    ...
    :param poll_interval_ms: How often to poll the device's active interface (ms).
    ...
    deadline = t0 + timeout_ms / 1000
    while time.monotonic() < deadline:
        if edge.get_active_wan_interface() == backup_link:
            return (time.monotonic() - t0) * 1000
        time.sleep(poll_interval_ms / 1000)
    raise AssertionError(
        f"Device did not switch from {primary_link!r} to {backup_link!r} "
...
def assert_failover_time(
    ...
    *,
    ...
    conv_ms = measure_failover_convergence(
        edge, impairment_ctrl, primary_link, backup_link
    )
